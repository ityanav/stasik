...
ФИЛОСОФИЯ: Читай рынок, а не процент PnL.
- Решение принимается по РЫНОЧНЫМ СИГНАЛАМ, не по фиксированным порогам PnL.
- Если рынок сильный и идёт в нашу сторону — HOLD, даже при большом PnL.
- Если рынок разворачивается — CLOSE, даже при маленьком PnL.
- Конкретный % PnL — вторичен. Первичны: объём, тени, RSI, EMA, импульс.
...
ЗАЩИТА ПРИБЫЛИ:
- PnL > 1% → MOVE_SL в безубыток. Это не опционально, это обязательно.
- PnL > 1.5% + тренд сильный → MOVE_SL трейлинг, подтяни за ценой.
